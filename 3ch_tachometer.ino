...
const int UPDATE_INTERVAL_MS = 100;   // Change this to any interval (20, 50, 100, 1000...)
...
void loop() {
  ...
  if (currentTime - past_time >= UPDATE_INTERVAL_MS) {
    ...
    // Calculate RPM
    float rpm1 = (p1 * 60000.0) / (PULSES_PER_REV * UPDATE_INTERVAL_MS);
    float rpm2 = (p2 * 60000.0) / (PULSES_PER_REV * UPDATE_INTERVAL_MS);
    float rpm3 = (p3 * 60000.0) / (PULSES_PER_REV * UPDATE_INTERVAL_MS);

    // Output as CSV (easy for LabVIEW)
    Serial.print(rpm1, 0);
    Serial.print(",");
    Serial.print(rpm2, 0);
    Serial.print(",");
